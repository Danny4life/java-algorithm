package ds.avlTree;

//Approach
//        An AVL tree is height-balanced if the depth of the two subtrees
//        of every node never differs by more than 1.
//        We use postorder traversal (bottom-up) to calculate the height
//        of each node while checking for balance.
//        If any subtree is unbalanced, return false.

public class TreeNode{
    int val;
    TreeNode left;
    TreeNode right;

    public TreeNode(int val) {
        this.val = val;
        this.left = null;
        this.right = null;
    }
}

class Solution {
    public boolean isBalanced(TreeNode root){

        return checkedHeight(root) != -1;

    }

    private int checkedHeight(TreeNode node) {
        if(node == null) return 0; // Base case: Empty tree is balanced

        // Recursively check height of left subtree
        int leftHeight = checkedHeight(node.left);
        if(leftHeight == -1) return -1; // If left subtree is unbalanced, return -1

        // Recursively check height of right subtree
        int rightHeight = checkedHeight(node.right);
        if(rightHeight == -1) return -1; // If right subtree is unbalanced, return -1

        // Check if the current node is balanced
        if(Math.abs(leftHeight - rightHeight) > 1) return -1;

        // Return height of the current node
        return Math.max(leftHeight, rightHeight) + 1;

    }


    public static void main(String[] args) {
        Solution solution = new Solution();

        // Example 1: Balanced Tree
        TreeNode root1 = new TreeNode(3);
        root1.left = new TreeNode(9);
        root1.right = new TreeNode(20);
        root1.right.left = new TreeNode(15);
        root1.right.right = new TreeNode(7);
        System.out.println(solution.isBalanced(root1)); // Output: true

        // Example 2: Unbalanced Tree
//        TreeNode root2 = new TreeNode(1);
//        root2.left = new TreeNode(2);
//        root2.right = new TreeNode(2);
//        root2.left.left = new TreeNode(3);
//        root2.left.right = new TreeNode(3);
//        root2.left.left.left = new TreeNode(4);
//        root2.left.left.right = new TreeNode(4);
//        System.out.println(solution.isBalanced(root2)); // Output: false

        // Example 3: Empty Tree
        //System.out.println(solution.isBalanced(null)); // Output: true
    }
    }


/**
 * Explanation
 * Recursive function checkHeight(TreeNode node):
 *
 * Returns height of the node if it's balanced.
 * Returns -1 if an unbalanced subtree is found.
 * Base case:
 *
 * If node == null, return height 0 (empty tree is balanced).
 * Recursive calls:
 *
 * Compute height of left and right subtrees.
 * If either subtree is unbalanced (-1 is returned), propagate -1 up.
 * Check balance condition:
 *
 * If the height difference |leftHeight - rightHeight| > 1, return -1 (unbalanced).
 * Otherwise, return max(leftHeight, rightHeight) + 1.
 * Final check in isBalanced(root):
 *
 * If checkHeight(root) == -1, return false (tree is not balanced).
 * Otherwise, return true.
 * Time Complexity
 * O(N) where N is the number of nodes.
 */

