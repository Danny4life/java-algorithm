...
public class SunsetViews {
    ...
    public static ArrayList<Integer> sunsetViews(int[] buildings, String direction) {
        ...
        // Determine the starting index and step based on the direction
        int startIndex = buildings.length - 1;
        int step = -1;


        // Set the starting index and step for EAST direction
        if (direction.equals("EAST")) {
            startIndex = 0;
            step = 1;
        }

        // Initialize an index to iterate through the buildings
        int index = startIndex;

        // Iterate through the buildings in the specified direction
        while (index >= 0 && index < buildings.length) {
            // Get the height of the current building
            int building = buildings[index];

            // Remove the indexes of buildings that cannot see the sunset due to the current building's height
            while (result.size() > 0
                    && buildings[result.get(result.size() - 1)] <= building) {
            ...
            // Move to the next building according to the specified step
            index += step;
        }
