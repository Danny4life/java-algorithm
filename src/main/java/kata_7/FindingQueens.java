...
//Your task is to find the maximum number of queens that can be put on the board
//        so that there would be one single unbeaten square (ie. threatened by no queen on the board).
//
...
public class FindingQueens {
    public static void main(String[] args) {
        ...
        System.out.println(queens(BigInteger.valueOf(0)));

    }
        public static BigInteger queens(BigInteger n) {
            if(n.compareTo(BigInteger.valueOf(0))<1)return BigInteger.valueOf(0);
            BigInteger c = n.multiply(BigInteger.valueOf(3)).subtract(BigInteger.valueOf(2));
            return n.pow(2).subtract(c);
        }
}
